...
class MCMCWrapper(object): # fix arg names before running idiot
	def __init__(self, sampler, theta, abscissastring, three, Nmcmc, Nburnin, Nens, ndim, emailaddress, outdir, modelStrings, argslist):
		...
		self.resultsaddress = "http://localhost/results/" + self.outdir
	...
	def run_postproc(self):
		if self.errval == 0:
			try:
				postprocessing(samples, theta, abscissastring, abscissastring, data, emailaddress, resultsaddress)
			except:
		...
		if self.errval != 0:
			...
			errorpage(self.errval, self.emailaddress, self.resultsaddress)
			success = False

		database_add_row(self.outdir, self.modelStrings, self.theta, len(self.theta), success)
